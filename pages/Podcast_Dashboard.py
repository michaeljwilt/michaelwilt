...
def generate_data(start_date, end_date):
    ...
    for i in range(num_days):
        ...
        prev_year_listen_time = random.randint(
            *weekday_range if prev_year_date.dayofweek < 5 else weekend_range
        )
...
# ---------------App Build---------------#
_, col1, col2 = st.columns([2, 0.5, 0.25])
col1.markdown("*Click for more info 👉*")
with col2.popover("ℹ️"):
    st.subheader("👉Overview")
    st.markdown(
        """*This dashboard is designed to provide a high-level overview of the key metrics for our podcast.*"""
    )
    st.write(" ")
    st.subheader("📍Data Sources")
    st.markdown(
        """The data is randomly created data to protect the privacy of the company this was built for."""
    )
    st.write(" ")
    st.subheader("📍Definitions")
    st.markdown(
        """**Listen Time:** The total number of minutes that users have listened to the podcast."""
    )
    st.markdown(
        """**Listeners:** The total number of verified listeners who have listened to the podcast. This means apple and spotify verified their devices."""
    )
    st.markdown(
        """**Downloads:** The total number of downloads of the podcast episodes."""
    )
# ---------------Title---------------#
st.markdown(
    """
...
col1, _, col3 = st.columns([0.5, 1, 0.5])
...
with st.container(border=True):
    ...
    col1.metric(
        "Total Listen Time", f"{current_listen_time:,}", f"↑ {listen_time_diff:.0f}%"
    )
    col2.metric("Total Listeners", f"{current_listeners:,}", f"↑ {listeners_diff:.0f}%")
    col3.metric(
        "Total Downloads", f"{int(current_downloads):,}", f"↑ {downloads_diff:.0f}%"
    )
...
def fragment():
    selected = option_menu(
        ...
        styles={"nav-link-selected": {"background-color": "#409cf4"}},
    )

    with st.container(border=True):
        if selected == "Listen Time":
            fig = px.line(
                fig_df, x="date", y="listen_time", title="Listen Time"
            )  # line chart

        elif selected == "Listeners":
            fig = px.line(
                fig_df, x="date", y="listeners", title="Listeners"
            )  # line chart

        elif selected == "Downloads":
            fig = px.line(
                fig_df, x="date", y="downloads", title="Downloads"
            )  # line chart
